...
class Bank:
    ...
    def initialize_csv(cls):
        if not os.path.exists(cls.file_path):
            with open(cls.file_path, "w", newline='') as file:
                writer = csv.DictWriter(file, fieldnames=["username", "password", "salt", "balance"])
                writer.writeheader()
    ...
    def sign_up(cls, username, password):
        ...
        with open(cls.file_path, "a", newline='') as file:
            writer = csv.DictWriter(file, fieldnames=["username", "password", "salt", "balance"])
            writer.writerow({"username": username, "password": hashed_password, "salt": salt, "balance": 0})  # initial balance is 0
    ...
    def login(cls, username, password):
        with open(cls.file_path, "r") as file:
            ...
            for row in reader:
                if row["username"] == username and cls.check_password(password, row["password"], row["salt"]):
